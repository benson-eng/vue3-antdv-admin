...
import { getStationMasters, queryStationMaster } from '@/api/backend/agentHubManager';
...
const { t } = useI18n('page.distAcctStationRecord');
const userStore = useUserStore();

const permissionsLevel = 3;
...
interface StationRowData extends StationMasterItem {
  pk?: string;
}
  ...
  data.forEach((item: any) => {
    ...
    switch (item.type) {
      case 'GameWinLose':
        ...
        type4Total.value += Number.parseFloat(one.masterAgentAmount || 0);
        break;
      case 'Purchase':
        reportType2Data.value.push(one);
        type2Total.value += Number.parseFloat(one.masterAgentAmount || 0);
        break;
      case 'Bonus':
        one.masterAgentAmount = (Number.parseFloat(one.masterAgentAmount || 0) * -1).toString().replace(/^-0$/, '0');
        one.total = (Number.parseFloat(one.total || 0) * -1).toString().replace(/^-0$/, '0');
        reportType5Data.value.push(one);
        type5Total.value += Number.parseFloat(one.masterAgentAmount || 0);
        break;
      case 'Transfer':
        break;
      case 'ServiceFee':
        reportType6Data.value.push(one);
        type6Total.value += Number.parseFloat(one.masterAgentAmount || 0);
        break;
      ...
      case 'PlatformMonthlyFee':
        ...
        type1Total.value += Number.parseFloat(one.masterAgentAmount || 0);
        break;
    }
  });
};
...
const getDataList = async (type: 'Payable' | 'Receivable') => {
  console.log('[getDataList] Called with type:', type);
  ...
  console.log('[getDataList] postData:', postData);
  billPrintDate.value = `${formatDate(queryForm.value.date[0].toDate())} ~ ${formatDate(queryForm.value.date[1].toDate())}`;

  // 先打開視窗，無論是否有資料都要顯示
  console.log('[getDataList] Opening dialog first...');
  isDialog.value = true;
  console.log('[getDataList] Dialog opened, isDialog.value:', isDialog.value);

  try {
    console.log('[getDataList] Calling financialSystemGetData...');
    const res = await financialSystemGetData(postData);
    console.log('[getDataList] res:', res);

    // Vue3 實際回傳格式：{ items: [...] }
    // 只使用 res.items 作為資料來源
    const items = (res as any)?.items;
    console.log('[getDataList] items:', items);

    // 僅在 Array.isArray(items) 時呼叫 dataInPage(items)
    if (Array.isArray(items)) {
      console.log('[getDataList] items 是數組，length:', items.length);
      dataInPage(items);
    }
    else {
      console.warn('[getDataList] items 不是數組或不存在，清空所有報表數據。items:', items);
      dataInPage([]);
    }
  }
  catch (error) {
...
const getGameList = async () => {
  console.log('[getGameList] Called with station.value:', station.value);
  gameNameList.value = {};
  try {
    console.log('[getGameList] Calling fetchGameList...');
    const res = await fetchGameList({ masterAgent: station.value });
    console.log('[getGameList] Response received:', res);
    if (res) {
      ...
      console.log('[getGameList] Processed gameNameList:', gameNameList.value);
    }
    else {
      console.warn('[getGameList] No response data');
    }
  }
  catch (error) {
...
const onEditBtnHandler = async (data: StationMasterItem, type: 'Payable' | 'Receivable') => {
  console.log('[onEditBtnHandler] Called with:', { data, type });
  if (!data || !data.name) {
    console.error('[onEditBtnHandler] invalid data', data);
    return;
  }
  console.log('[onEditBtnHandler] Setting station to:', data.name);
  station.value = data.name;
  console.log('[onEditBtnHandler] Calling getGameList...');
  await getGameList();
  console.log('[onEditBtnHandler] Calling getDataList with type:', type);
  await getDataList(type);
  console.log('[onEditBtnHandler] Completed, isDialog.value:', isDialog.value);
};
...
const [DynamicTable, dynamicTableInstance] = useTable({
  ...
  actionColumn: {
    ...
    fixed: 'right',
    actions: ({ record }) => {
      console.log('[actionColumn] actions called, record:', record);
      return [
        {
          label: t('label.viewTable') || '查看報表',
          type: 'link',
          onClick: (params) => {
            console.log('[actionColumn] onClick triggered, params:', params);
            const currentRecord = params?.record || record;
            console.log('[actionColumn] onClick triggered, currentRecord:', currentRecord);
            if (currentRecord) {
              console.log('[actionColumn] Calling onEditBtnHandler with:', currentRecord, 'Payable');
              onEditBtnHandler(currentRecord as StationMasterItem, 'Payable');
            }
            else {
              console.warn('[actionColumn] onClick: record is null or undefined');
            }
          },
        },
      ];
      ...
      if (response && response.result === true && response.value) {
        response.value.forEach((item: StationMasterItem) => {
          ...
          stationList.value.push({
            ...item,
            value: item.name,
            pk: item.name || `station-${item.id || Math.random()}`,
          });
        });
      }
    }
    ...
    try {
      ...
      if (response && response.result && response.value) {
        stationList.value.push({
          ...
          pk: response.value.name || `station-${response.value.id || Math.random()}`,
        });
        station.value = response.value.name || '';
      }
    }
    catch (error) {
      console.error('Failed to query station master:', error);
    }
  }
...
const loadTableData = async () => {
  ...
  const items = stationList.value.map(item => ({
    ...item,
    pk: item.pk || item.name || `station-${item.id || Math.random()}`,
  }));

  return {
    items,
    meta: {
      totalItems: stationList.value.length,
    },
  };
};

const columns = ref<TableColumn<StationRowData>[]>([
  {
    title: t('label.station') || '站台',
    dataIndex: 'name',
    width: 200,
  },
]);
...
<template>
  <div class="app-container dist-acct-station-record-table">
        ...
        <div id="printArea" class="bill-container">
          <div class="bill-section">
            <h2 class="bill-header">
              <span>{{ t('label.target') || '對象' }}：{{ station }}</span>
              <span class="">
          ...
          <div class="bill-section">
            ...
            <div v-if="reportType1Data.length > 0" class="bill-table">
              ...
              <div v-for="item in reportType1Data" :key="item.id" class="row">
                ...
                <div class="cell right-align">
                  {{ formatNumber((item.masterAgentRate || 0) * 100) }}%
                </div>
                <div class="cell right-align">
...
.dist-acct-station-record-table {
  .wrap {
    display: flex;
    flex-wrap: wrap;
